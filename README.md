...
- **右键菜单**——重命名、删除、复制路径
- **快捷键**——Ctrl+S 保存等
- **只读模式**——`--readonly` 参数禁止所有写操作，适合安全浏览场景
...
```bash
npx edlics serve --hostname 0.0.0.0 --port 5000 --readonly
```
...
| **VS Code 风格标签栏** | 横向滚动标签栏，关闭按钮悬浮背景，预览切换和刷新按钮固定在右侧 |
| **文件刷新** | 标签栏刷新按钮，一键重新从磁盘加载文件内容 |
| **智能搜索** | 自动检测项目类型（Node.js、Go、Java、C#、Python、Rust、Ruby），排除构建/缓存目录 |
| **文件操作** | 创建、重命名、删除文件和文件夹——右键菜单 |
| **只读模式** | `--readonly` 参数禁止所有写操作，隐藏写操作菜单，Ctrl+S 显示提示 |
...
使用 `--readonly` 参数启动时，服务器进入只读模式：

- 禁止所有写操作（保存、创建、重命名、删除、上传）
- 前端自动隐藏写操作相关的菜单项
- 标签栏显示 "readonly" 标识

适用于安全浏览场景，如公开演示或只读查看服务器配置。
...
8. 如果设置了 `--readonly`，所有写操作被禁止——只能查看和下载文件
...
本项目基于 [opermancode/edlics](https://github.com/opermancode/edlics) 二次开发，在此感谢原作者 **opermancode** 的开源贡献。

Edlics-Plus 在原版基础上进行了大量功能扩展和架构重构，包括模块化前端、14 种语言语法高亮、Markdown/SVG 预览、智能搜索过滤、只读模式等。
