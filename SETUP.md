...
`.github/workflows/publish.yml` 会在 tag `v*` 推送时自动：设置版本 → `npm install` → `npm run build` → **`npm test` 硬闸门（全绿才继续，失败即停）** → 冒烟测试 → 生成 CHANGELOG → 通过 **OIDC Trusted Publishing** 发布到 npm（无需手动配置 token）→ **自动创建同名 GitHub Release**（正文为 CHANGELOG）。
